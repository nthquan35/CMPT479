...
void login2(char * input1, char * input2) {
	...
	// // printf("%p", v.good_username);
	// // printf("\n");
	// printf("real username is: %s\n", v.good_username);
	// printf("real password is: %s\n", v.good_password);

	// printf("%d\n", v.canary);
	// printf("%d\n", v.goodcanary);

	// if (strcmp(v.password, v.good_password) == 0){
	// 	printf("TRUE\n");
	// }
...
void login3(char * input1, char * input2) {
	...
	while (written_char < 25) { //don't write too much
		...
		else if (c == 58 || c == 59) {//sometimes user types some characters by accident. let's remove them. 
			v.password[ind] = 0;
			written_char += 1;
		}
